CREATE INDEX IF NOT EXISTS idx_deadlines_completed_tenant_concluido_em
  ON public.deadlines (tenant_id, completed, concluido_em DESC)
  WHERE completed = true;

CREATE OR REPLACE FUNCTION public.get_central_prazos_concluidos(
  p_tenant_id uuid,
...
AS $$
  ...
  FROM (
    SELECT
      ...
      b.protocolo_etapa_id, b.project_id,
      ...
      COALESCE(subs.subtarefas, '[]'::jsonb) AS subtarefas
    FROM base b
    ...
    LEFT JOIN subs ON subs.deadline_id = b.id
  ) t;
$$;

GRANT EXECUTE ON FUNCTION public.get_central_prazos_concluidos(uuid, integer, uuid) TO authenticated;
...
AS $$
  ...
  FROM (
    SELECT
      ...
      ELSE NULL END AS protocolo_etapa,
      pw.nome AS "workspaceName",
      CASE WHEN po.id IS NOT NULL THEN
        jsonb_build_object(
          'numeroCnj', COALESCE(po.numero_cnj, ''),
          'parteAtiva', COALESCE(po.parte_ativa, ''),
          'partePassiva', COALESCE(po.parte_passiva, '')
        )
      ELSE NULL END AS "processoInfo",
      COALESCE(subs.subtarefas, '[]'::jsonb) AS subtarefas
    FROM base b
    LEFT JOIN public.profiles pa ON pa.user_id = b.advogado_responsavel_id
    LEFT JOIN public.profiles pc ON pc.user_id = b.concluido_por
    ...
    LEFT JOIN public.project_protocolo_etapas pe ON pe.id = b.protocolo_etapa_id
    LEFT JOIN public.project_protocolos pp ON pp.id = pe.protocolo_id
    LEFT JOIN public.project_workspaces pw ON pw.id = b.workspace_id
    LEFT JOIN public.processos_oab po ON po.id = b.processo_oab_id
    LEFT JOIN subs ON subs.deadline_id = b.id
  ) t;
$$;
