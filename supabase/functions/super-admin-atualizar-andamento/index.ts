...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { andamento_id, sigiloso, tribunal_tag } = parsed.data;

    const { data: and, error: gErr } = await admin.from('processos_oab_andamentos').select('id, dados_completos').eq('id', andamento_id).maybeSingle();
    if (gErr) throw gErr;
    if (!and) return new Response(JSON.stringify({ error: 'not_found' }), { status: 404, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });

    const dc: Record<string, unknown> = { ...((and.dados_completos as any) || {}) };
    if (sigiloso !== undefined) dc.sigiloso = !!sigiloso;
    if (tribunal_tag !== undefined) dc.tribunal_tag = tribunal_tag || null;

    const { error: uErr } = await admin.from('processos_oab_andamentos').update({ dados_completos: dc }).eq('id', andamento_id);
    if (uErr) return new Response(JSON.stringify({ error: 'update_falhou', detail: uErr.message }), { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
