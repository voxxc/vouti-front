...
const Body = z.discriminatedUnion('action', [
  z.object({ action: z.literal('criar'), nome: z.string().trim().min(1).max(80), cor: z.string().trim().max(20).optional().nullable() }),
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (b.action === 'criar') {
      ...
      const { data, error } = await admin.from('super_admin_sistemas_processo').insert({ slug, nome: b.nome, cor: b.cor || null, created_by: userRes.user.id }).select('id, slug, nome, cor').single();
      ...
      return new Response(JSON.stringify({ ok: true, sistema: data }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    }
    if (b.action === 'editar') {
      const patch: Record<string, unknown> = {};
      if (b.nome !== undefined) patch.nome = b.nome;
      if (b.cor !== undefined) patch.cor = b.cor;
      const { data, error } = await admin.from('super_admin_sistemas_processo').update(patch).eq('id', b.id).select('id, slug, nome, cor').single();
    ...
    const { error } = await admin.from('super_admin_sistemas_processo').delete().eq('id', b.id);
