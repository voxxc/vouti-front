...
import { ReuniaoFormData, HORARIOS_DISPONIVEIS } from '@/types/reuniao';
...
interface ReuniaoFormWrapperProps {
...
export const ReuniaoFormWrapper = ({ initialData, onSubmit, onCancel, isSubmitting }: ReuniaoFormWrapperProps) => {
  const { status: statusOptions, loading: loadingStatus } = useReuniaoStatus();
  const activeStatus = statusOptions.filter(s => s.ativo);
  ...
  const form = useForm<any>({
    ...
    defaultValues: {
      ...
      status: initialData?.status || (activeStatus[0]?.nome || '1ª reunião'),
      observacoes: initialData?.observacoes || ''
    }
  });

  const handleSubmit = async (data: any) => {
    await onSubmit(data as ReuniaoFormData);
  };
