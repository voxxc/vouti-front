...
import { Link2, Unlink, Search, ExternalLink, Loader2 } from 'lucide-react';
...
interface TaskVinculoTabProps {
  taskId: string;
  processoOabId: string | null | undefined;
  onVinculoChange: (novoProcessoOabId: string | null) => void;
}

export const TaskVinculoTab = ({ taskId, processoOabId, onVinculoChange }: TaskVinculoTabProps) => {
  ...
  const {
    ...
    vincularProcesso,
    desvincularProcesso,
  } = useTaskVinculo(taskId, processoOabId);
  ...
  const handleVincular = async (processoId: string) => {
    const success = await vincularProcesso(processoId);
    if (success) {
      onVinculoChange(processoId);
        ...
        <Card>
          <CardContent className="p-4 space-y-3">
            ...
            <div className="grid grid-cols-2 gap-2 text-sm">
              ...
              <div>
                <span className="text-xs text-muted-foreground block">Reu</span>
    ...
    <div className="space-y-4">
      ...
      {!mostrarBusca ? (
        <div className="text-center py-8 text-muted-foreground">
          ...
          <p className="text-xs">Clique em "Buscar Processo" para vincular este card a um processo da Controladoria</p>
        </div>
      ) : (
        <div className="space-y-3">
          <div className="flex gap-2">
            <Input
              placeholder="Buscar por CNJ, autor ou reu..."
              value={busca}
                    ...
                    <CardContent className="p-3">
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        <span className="text-xs text-muted-foreground truncate flex-1">
                          {processo.parte_ativa || 'Autor'} x {processo.parte_passiva || 'Reu'}
                        </span>
